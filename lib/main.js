class BlendedMap {
    ...
    outline(size=1, layers=1, whitelist=null) {
        ...
        const inLastDepth = (value, nvalue) => {
            ...
            let nval   = typeof nvalue == "string"? nvalue.split('-')[0]: nvalue;

            return pval == nval && pdepth == 0;
        };
        const lookEvery = (val) => {
        ...
        let iterate_on = whitelist || this;
        iterate_on.forEach(value => {
            for(let s=0; s < size; s++) {
                lookEvery(value);
            }
        });

        return this.outline(size, layers-1, whitelist);
    }
